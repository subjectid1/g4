...
public class ProjectInfoAutoConfiguration__BeanDefinitions {
  ...
  /**
   * Get the bean instance supplier for 'gitProperties'.
   */
  private static BeanInstanceSupplier<GitProperties> getGitPropertiesInstanceSupplier() {
    return BeanInstanceSupplier.<GitProperties>forFactoryMethod(ProjectInfoAutoConfiguration.class, "gitProperties")
            .withGenerator((registeredBean) -> registeredBean.getBeanFactory().getBean("org.springframework.boot.autoconfigure.info.ProjectInfoAutoConfiguration", ProjectInfoAutoConfiguration.class).gitProperties());
  }

  /**
   * Get the bean definition for 'gitProperties'.
   */
  public static BeanDefinition getGitPropertiesBeanDefinition() {
    RootBeanDefinition beanDefinition = new RootBeanDefinition(GitProperties.class);
    beanDefinition.setFactoryBeanName("org.springframework.boot.autoconfigure.info.ProjectInfoAutoConfiguration");
    beanDefinition.setInstanceSupplier(getGitPropertiesInstanceSupplier());
    return beanDefinition;
  }
}
